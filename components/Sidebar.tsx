...
export default function Sidebar({ userRole, userName, userEmail, avatarUrl, currentTier }: SidebarProps) {
  ...
  const renderSidebarContent = (compact = false) => (
    <>
      {/* Logo */}
      <div className={`h-14 flex items-center gap-2.5 px-4 ${compact ? 'justify-center' : ''}`} style={{ borderBottom: '1px solid var(--sidebar-border)' }}>
        <div
      ...
      {/* User Footer */}
      {!compact && <div ref={accountMenuRef} className="relative p-3" style={{ borderTop: '1px solid var(--sidebar-border)' }}>
